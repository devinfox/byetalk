-- Reduce turbo mode retry delay from 5 minutes to 30 seconds
-- This allows faster retrying of failed/busy/no-answer calls
...
BEGIN
  ...
  WITH available_leads AS (
    ...
    WHERE tcq.organization_id = p_organization_id
      ...
      AND (tcq.last_attempt_at IS NULL OR tcq.last_attempt_at < NOW() - INTERVAL '30 seconds')
      AND tcq.attempts < 3
